#!/usr/bin/env python3
"""ollama_plan_formatter.py
================================
Solicita un plan al modelo de Ollama en formato **JSON estricto** y lo re‑imprime
como bloque de texto con el estilo `PLAN:`.  Además registra todo en disco.

Pasos principales
-----------------
1. Lista los modelos locales de Ollama y deja que el usuario escoja uno.
2. Envía el *prompt* con `options={"format":"json", "temperature":0}` para
   forzar salida JSON y reducir la aleatoriedad.
3. **Reúne** todos los *chunks* del `stream=True` en una sola cadena
   (`raw_json`).
4. Intenta `json.loads(raw_json)`.  Si falla, realiza una limpieza rápida
   (extrae desde la primera «{» hasta la última «}») e intenta de nuevo.
5. Analiza el objeto y genera el bloque `PLAN:`.
6. Guarda `raw_json`, el bloque plan y metadatos en `log.txt` y `log.json`.

Uso rápido
~~~~~~~~~~
:::
$ python3 ollama_plan_formatter.py
1: llama3
2: mistral
Selecciona el número del modelo que quieres usar: 1
...           # se muestra la respuesta JSON
PLAN:
    0.000: (start_welcome tiago)  [1.000]
    1.001: (move tiago home dibejo)  [15.000]
    ...
:::

"""

import json
import sys
from datetime import datetime
from pathlib import Path

import ollama

# ------------------------------------------------------------
#  Utilidades
# ------------------------------------------------------------


# ---------- Helper functions -------------------------------------------------

def build_prompt(domain: str, problem: str) -> str:
    """Return a system prompt that forces a JSON‑only answer.

    The model must return **exactly** one JSON object with the key
    `final_plan`, whose value is a list of steps.  Each step is an object
    with:
        time (number) – start time in seconds with 3 decimals
        action (string) – name of the durative action
        params (array) – list of string parameters (may be empty)
        duration (number) – duration in seconds with 3 decimals

    A second key `explanation` contains free‑form text explaining why each
    step (especially recharges and explanations) is present.  Everything
    must be valid JSON, no extra text.
    """

    return f"""
    Eres un planificador PDDL.
    Devuelve **solo** un objeto JSON con esta estructura:
    {{
    "explanation": "texto libre explicando cada paso y cuándo recargar, por qué explicas un cuadro y si estaba en el goal o no",
    "final_plan": [{{
        "time": 0.000,
        "action": "start_welcome",
        "params": ["tiago"],
        "duration": 1.000
    }}]
    }}
    No añadas ningún otro texto.

    Considera la batería (100 al inicio, −20 por movimiento, −10 por explicación).
    Recarga **solo** en ubicaciones con cargador y solo cuando sea imprescindible.
    No inventes acciones ni hagas movimientos o recargas innecesarias.

    Estos son el dominio y el problema:

    DOMINIO:
    """
    {domain}
    """
    PROBLEMA:
    """
    {problem}
    """
    """

# ---------- Load domain & problem files --------------------------------------

# Adjust the paths as needed
DOMAIN_PATH = Path("/home/jfisher/tfg/pddl/domain.pddl")
PROBLEM_PATH = Path("/home/jfisher/tfg/pddl/problem.pddl")

with DOMAIN_PATH.open() as f:
    domain_text = f.read()

with PROBLEM_PATH.open() as f:
    problem_text = f.read()

def choose_model(client: ollama.Client) -> str:
    """Lista modelos locales y devuelve el nombre elegido."""
    models = client.list()
    print("Modelos disponibles:\n")
    for idx, mdl in enumerate(models["models"], start=1):
        print(f"{idx}: {mdl['model']}")
    choice = int(input("Selecciona el número del modelo que quieres usar: "))
    return models["models"][choice - 1]["model"]


def collect_stream(stream) -> str:
    """Concatena todos los `part['response']` del generador."""
    chunks: list[str] = []
    for part in stream:
        chunk = part.get("response", "")
        print(chunk, end="", flush=True)  # Mostrar en tiempo real
        chunks.append(chunk)
    return "".join(chunks).strip()


def safe_json_loads(raw: str):
    """Intenta cargar JSON; si falla, limpia bordes y re‑intenta."""
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
        start, end = raw.find("{"), raw.rfind("}")
        if start != -1 and end != -1 and start < end:
            cleaned = raw[start : end + 1]
            return json.loads(cleaned)
        raise  # Propaga si sigue fallando


def render_plan(final_plan: list[dict]) -> str:
    """Convierte la lista de pasos a bloque PLAN."""
    lines = ["PLAN:"]
    for step in final_plan:
        time = float(step["time"])
        action = step["action"]
        params = " ".join(step.get("params", []))
        duration = float(step["duration"])
        lines.append(f"    {time:.3f}: ({action} {params})  [{duration:.3f}]")
    return "\n".join(lines)


# ------------------------------------------------------------
#  Script principal
# ------------------------------------------------------------

def main():
    client = ollama.Client()
    model = choose_model(client)
    print(f"Has seleccionado el modelo: {model}\n")

    # --- Construir prompt ---
    # prompt = Path("prompt.txt").read_text(encoding="utf-8") if Path("prompt.txt").exists() else input("Pegue aquí su prompt completo y termine con Ctrl‑D:\n")
    # prompt = 
    prompt = f"""
You are a planner for PDDL and I only want you to give me the execution plan. You need to take into account the following:

1.The robot has a battery that is consumed with each movement and action.
2.The battery must be recharged when necessary, but you should minimize the number of recharges and movements to make the plan as efficient as possible.
3.Maximize efficiency regarding battery usage, performing recharges only when absolutely necessary and only at locations that have chargers.
4.Do not make any comments or additional explanations. I only want the plan.
5.Do not invent actions or make unnecessary actions.

I will provide you with the domain and the problem. The only output I expect from you is the execution plan, as a set of actions, with their start times and durations. The structure of the plan should be the following:

0.000: (start_welcome tiago) [1.000]
1.001: (move tiago home dibejo) [15.000]
16.002: (move tiago dibejo elgrito) [15.000]
31.002: (explain_painting tiago elgrito) [15.000]
46.002: (move tiago elgrito guernica) [15.000]
...
Now I will provide the domain and the problem. ONLY RESPOND WITH THE PLAN, NOTHING ELSE.

dominio :
"
(define (domain library_domain)

(:requirements :strips :fluents :durative-actions :typing)

(:types 
  robot
  location 
  painting
)

(:predicates
  (robot_at ?r - robot ?l - location)
  (painting_at ?b - painting ?l - location)
  (painting_found ?b - painting)
  (explained_painting ?p - location)
  (can_start ?r - robot)
  (initial_state ?r - robot)
  (visited ?r - robot ?l - location)
  (charger_at ?wp - location)
)

(:functions
  (battery ?r - robot)
)

(:durative-action start_welcome
  :parameters (?r - robot)
  :duration (= ?duration 1)
  :condition (and 
    (at start (initial_state ?r))
  )
  :effect (and 
    (at end (can_start ?r))
    (at start (initial_state ?r))
  )
)

(:durative-action move
  :parameters (?r - robot ?from ?to - location)
  :duration (= ?duration 15)
  :condition (and 
    (at start (>= (battery ?r) 20))
    (at start (can_start ?r))
    (at start (robot_at ?r ?from))
  )
  :effect (and 
    (at start (not(robot_at ?r ?from)))
    (at start (decrease (battery ?r) 20))
    (at end (robot_at ?r ?to))
    (at end (visited ?r ?to))
  )
)

(:durative-action explain_painting
  :parameters (?r - robot ?p - location)
  :duration (= ?duration 15)
  :condition (and 
    (at start (>= (battery ?r) 10))
    (over all (robot_at ?r   ?p))
    (at start (can_start ?r))
  )
  :effect (and 
    (at end (explained_painting ?p)) 
    (at start (decrease (battery ?r) 10))
  )
)

(:durative-action recharge
  :parameters (?r - robot ?wp - location)
  :duration (= ?duration 5)
  :condition (and 
    (at start (<= (battery ?r) 100))
    (over all (robot_at ?r ?wp))
    (over all (charger_at ?wp))
  )
  :effect (and (at end (assign (battery ?r) 100)))
)
"

problem:
"
(define (problem library_problem)
(:domain library_domain)

(:objects
 home monalisa nocheestrellada elgrito dibejo guernica la_joven_de_la_perla las_meninas el_3_de_mayo_de_1808 
  el_jardin_de_las_delicias las_tres_gracias la_rendicion_de_breda el_nacimiento_de_venus
  la_creacion_de_adan la_ultima_cena la_libertad_guiando_al_pueblo el_hijo_del_hombre american_gothic 
  la_persistencia_de_la_memoria la_ronda_de_noche impresion_sol_naciente banistas_en_asnieres 
  saturno_devorando_a_su_hijo whistlers_mother la_gran_ola_de_kanagawa el_beso autorretrato_con_collar_de_espinas 
  el_carnaval_del_arlequin nighthawks retrato_de_adele_bloch_bauer_i campbells_soup_cans composition_viii - location
  tiago - robot
)

(:init
  (robot_at tiago home)
  (initial_state tiago)
  (= (battery tiago) 100)
  (charger_at home)
)

(:goal
  (and
    (visited tiago la_libertad_guiando_al_pueblo)
    (explained_painting guernica)
  )
)
"

The most important thing in the plan is that you consider the battery.
Starting with 100, moving consumes 20, and explaining consumes 10. Therefore, you must recharge the robot before the battery runs out. If you have 20 battery, the action must be MOVE TO HOME to recharge.

REMEMBER, THE OUTPUT MUST ALWAYS HAVE THIS FORMAT.

0.000: (start_welcome tiago) [1.000]
1.001: (move tiago home dibejo) [15.000]
16.002: (move tiago dibejo elgrito) [15.000]
31.002: (explain_painting tiago elgrito) [15.000]
46.002: (move tiago elgrito guernica) [15.000]


Give me de step by step explanation of when you should recharge de robot, when you are planning and explaining a painting,
tell me why you are explaining the drawing and if it was in the goal or not. 

But at the end do:
"
PLAN: 
(AND GIVE THE PLAN WITH CORRECT FORMAT)
"
"""

    # --- Llamada al modelo (JSON estricto) ---
    stream = client.generate(
        model=model,
        prompt=prompt,
        stream=True,
        options={"format": "json", "temperature": 0},
    )

    # --- Reunir chunks ---
    raw_json = collect_stream(stream)

    # --- Parsear JSON ---
    payload = safe_json_loads(raw_json)
    final_plan = payload.get("final_plan", [])

    # --- Renderizar bloque PLAN ---
    plan_block = render_plan(final_plan)
    print("\n" + plan_block)

    # --- Registro ---
    # now = datetime.now().isoformat(timespec="seconds")
    # log_txt = Path("log.txt")
    # log_json = Path("log.json")

    # # log_txt.write_text(f"\n[{now}]\n{plan_block}\n", encoding="utf-8", errors="ignore", append=True)

    # with log_json.open("a", encoding="utf-8") as f:
    #     json.dump({"timestamp": now, "model": model, "prompt": prompt, "response": payload}, f, ensure_ascii=False)
    #     f.write("\n")


if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        sys.exit(0)
