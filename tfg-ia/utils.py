...
def write_log_txt(file_name, model, prompt, response):
    """Escribe logs en formato texto en tfg_ollama/logs/"""
    # Crear directorio logs en tfg_ollama si no existe
    tfg_ollama_dir = Path(__file__).resolve().parent / "tfg_ollama"
    logs_dir = tfg_ollama_dir / "logs"
...
def write_log_json(file_name, model, prompt, response):
    """Escribe logs en formato JSON en tfg_ollama/logs/"""
    # Crear directorio logs en tfg_ollama si no existe
    tfg_ollama_dir = Path(__file__).resolve().parent / "tfg_ollama"
    logs_dir = tfg_ollama_dir / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
