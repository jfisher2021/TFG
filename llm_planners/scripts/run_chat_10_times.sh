...
# Determinar ruta relativa a la carpeta del script (proyecto llm_planners)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)" 

PYTHON_BIN="$PROJECT_ROOT/.venv/bin/python"
SCRIPT_PATH="$PROJECT_ROOT/langchain_planner/scripts_evaluacion/chat_flujo_completo.py"
