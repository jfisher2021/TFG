...
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("my_llm_plan_solver");
  ...
  auto planner = std::make_shared<plansys2::LLMPlanSolver>();
...
TEST(llm_plan_solver, generate_plan_good)
{
  test_plan_generation();
}

TEST(llm_plan_solver, generate_plan_good_with_argument)
{
  test_plan_generation("-e");
}

TEST(llm_plan_solver, load_popf_plugin)
{
  try {
    ...
    plansys2::PlanSolverBase::Ptr plugin =
      lp_loader.createUniqueInstance("plansys2/LLMPlanSolver");
    ASSERT_TRUE(true);
...
TEST(llm_plan_solver, check_1_ok_domain)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("my_llm_plan_solver");
...
TEST(llm_plan_solver, check_2_error_domain)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("my_llm_plan_solver");
...
TEST(llm_plan_solver, generate_plan_unsolvable)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("my_llm_plan_solver");
  std::ifstream domain_ifs(pkgpath + "/pddl/domain_simple.pddl");
  std::string domain_str((
...
TEST(llm_plan_solver, generate_plan_error)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("my_llm_plan_solver");
  std::ifstream domain_ifs(pkgpath + "/pddl/domain_simple.pddl");
  std::string domain_str((
...
TEST(llm_plan_solver, create_folder_default)
{
...
TEST(llm_plan_solver, create_folder_custom_path)
{
...
TEST(llm_plan_solver, create_folder_custom_path_and_namespace)
{
...
TEST(llm_plan_solver, create_folder_filesystem_error)
{
