...
#include "plansys2_tamer_plan_solver/tamer_plan_solver.hpp"
...
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("plansys2_tamer_plan_solver");
  ...
  auto planner = std::make_shared<plansys2::TAMERPlanSolver>();
  planner->configure(node, "TAMER");
  node->set_parameter(rclcpp::Parameter("TAMER.arguments", argument));
  ...
  const std::string & output_dir = "", const std::string & node_namespace = "")
{
  ...
  planner->configure(node, "TAMER");
  if (!output_dir.empty()) {
    node->set_parameter(rclcpp::Parameter("TAMER.output_dir", output_dir));
  }

  return planner->create_folders(node_namespace);
}

TEST(tamer_plan_solver, generate_plan_good)
{
  test_plan_generation();
}

TEST(tamer_plan_solver, generate_plan_good_with_argument)
{
  test_plan_generation("-e");
}

TEST(tamer_plan_solver, load_tamer_plugin)
{
  try {
    ...
    plansys2::PlanSolverBase::Ptr plugin =
      lp_loader.createUniqueInstance("plansys2/TAMERPlanSolver");
    ASSERT_TRUE(true);
...
TEST(tamer_plan_solver, check_1_ok_domain)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("plansys2_tamer_plan_solver");
...
TEST(tamer_plan_solver, check_2_error_domain)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("plansys2_tamer_plan_solver");
...
TEST(tamer_plan_solver, generate_plan_unsolvable)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("plansys2_tamer_plan_solver");
  std::ifstream domain_ifs(pkgpath + "/pddl/domain_simple.pddl");
  std::string domain_str((
...
TEST(tamer_plan_solver, generate_plan_error)
{
  std::string pkgpath = ament_index_cpp::get_package_share_directory("plansys2_tamer_plan_solver");
  std::ifstream domain_ifs(pkgpath + "/pddl/domain_simple.pddl");
  std::string domain_str((
...
TEST(tamer_plan_solver, create_folder_default)
{
...
TEST(tamer_plan_solver, create_folder_custom_path)
{
...
TEST(tamer_plan_solver, create_folder_custom_path_and_namespace)
{
...
TEST(tamer_plan_solver, create_folder_filesystem_error)
{
