...
TAMERPlanSolver::TAMERPlanSolver()
{
}

std::optional<std::filesystem::path>
TAMERPlanSolver::create_folders(const std::string & node_namespace)
{
...
void TAMERPlanSolver::configure(
  ...
  const rclcpp::Duration solver_timeout)
{
  ...
  RCLCPP_DEBUG(
    lc_node_->get_logger(),
    "[%s-tamer] called with timeout %f seconds with args [%s] with output dir %s",
    lc_node_->get_name(), solver_timeout.seconds(), args.c_str(), output_dir.c_str());

  bool success = execute_planner(tamer_exec_ + " " +
...
std::optional<plansys2_msgs::msg::Plan>
TAMERPlanSolver::parse_plan_result(const std::string & plan_path)
{
  std::string line;
  ...
  if (plan_file.is_open()) {
    while (getline(plan_file, line)) {
      if (!solution) {
        if (line.find("TimeTriggeredPlan:") != std::string::npos) {
          solution = true;
        }
      } else {
        plansys2_msgs::msg::PlanItem item;

        // Erase all comas and replace ( for a blank space
...
PLUGINLIB_EXPORT_CLASS(plansys2::TAMERPlanSolver, plansys2::PlanSolverBase);
