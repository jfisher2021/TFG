^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package plansys2_tamer_plan_solver
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* Add unit tests
* Do not segfault with filesystem errors in TAMER, and allow ~ for home directory
* Add option to use planner node to validate domain in domain expert
* Configure TAMER solver in domain expert
* Add ability to specify output folder in TAMER planner
...
* add status check into tamer_plan_solver
...
------------------
* Humble Upgrade
* Update tamer checker interface
...
* Added ability to provide command-line arguments to tamer via ROS parameter.
* Update version
